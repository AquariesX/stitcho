...
import {
    ...
    Loader2, FolderOpen, Layers, AlertTriangle
} from 'lucide-react';
...
export default function CategoriesPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            const result = await getCategories(role, user.id);
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        if (editCategory) {
            ...
            if (result.success && result.data) {
                setCategories(
                    categories.map((c) => (c.id === editCategory.id ? result.data : c))
                );
                closeModal();
            } else {
        ...
        <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <h1 className="text-3xl md:text-4xl font-bold tracking-tight">Categories</h1>
                        </div>
                        <p className="text-white/70 max-w-xl text-lg">
                            Organize your inventory with categories. Add, edit, or remove categories to keep your catalog clean.
                        </p>
                    </div>
                    <motion.button
            ...
            >
                ...
                <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-5 py-2.5 shadow-sm">
                    <FolderOpen size={18} className="text-[#223943]" />
                    ...
                    <AnimatePresence mode="popLayout">
                        {filteredCategories.map((category, i) => (
                            ...
                            >
                                ...
                                {/* Body */}
                                <div className="p-5">
                                    <h3 className="text-lg font-bold text-black truncate">{category.name}</h3>
                                    <div className="flex flex-wrap gap-2 mt-2">
                                        <span className="inline-block px-3 py-1 bg-[#223943]/10 text-[#223943] text-xs font-mono font-semibold rounded-lg">
                                            {category.code}
                                        </span>
                                        {role === 'admin' && category.user && (
                                            <span className="inline-block px-3 py-1 bg-amber-100 text-amber-800 text-xs font-semibold rounded-lg">
                                                By: {category.user.shopProfile?.shopName || category.user.name}
                                            </span>
                                        )}
                                    </div>
                                </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <p className="text-gray-500 text-sm mb-6">
                                This action cannot be undone. The category and its associations will be permanently removed.
                            </p>
                    ...
                    >
                        ...
                        >
                            ...
                            <form onSubmit={handleSubmit} className="p-8 space-y-6">
                                ...
                                {/* Image Upload */}
                                <div className="space-y-2">
                                    <label className="text-sm font-semibold text-black">Category Image</label>
                                    <div
